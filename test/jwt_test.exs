defmodule ELBCognitoPlug.JWTTest do
  use ExUnit.Case
  import ELBCognitoPlug.JWT
  doctest ELBCognitoPlug.JWT
  ...
  test "verify_with_key/2 returns claims if signature is correct" do
    ...
    {:ok, key} = MockCognitoKeys.get_cognito_jwk("key_id")
    ...
    assert {:ok, ^expected_claims} = verify_with_key(jwt, signer)
  end
  ...
  test "verify_with_key/2 returns error if key is incorrect" do
    ...
    signer = Joken.Signer.create(key["alg"], key)
    assert {:error, :signature_error} = verify_with_key(jwt, signer)
  end

  test "verify_cognito_jwt/2 pulls key from key module and verifies the JWT" do
    jwt =
      "eyJraWQiOiJrZXlfaWQiLCJhbGciOiJSUzI1NiJ9..bK2De82h7NCSYur5UcTNCqTOGZYt8Xw043c45j716QCND2kcbA7JHk71JNu5jxZbv6DUqmZYY8iauSKxZJYtn7ETHa0WI7x4uWv8qblUWygj-DYR3xf8aWj_QelXo9NXlvMCVXcbG-xn_Wu8rSbNwlIiod5F2gDA3xubhUOKUhZ51RGqidTo6l-r-slXKakmpvDVyWcwsUeeN7JXogDDa1-RA_wx0pGTVronmGpW_NV-fCRXsA35sGXgJ2Yj8B1wRPiwnfhN66BCr6UH8UHdut0v3K-MJMIEqh1MKGNfcTO7-K4tIavrbozcSgpBxfWbYS4odI_HZzCrruJFg6wfKGav5-h97pVdukuGEEolqj2-aNZ9PrdF4No7pjFxsa4str4-mxolwr0UxvTs58cn7bjKpP77yCQuI3FHoHb_V5NaFF_BUG6Q-JMh7tIoXjuA69_keZjBbmew80IU3jKDYa--wqHvrDs-hQqSe0LULW4ao-SGXceivxn1_YhxqmrpdN1HLIDcPuNJ8GTYJ6q1u2Fu45R7xK7deRlYDDiDrWCQhizslv8UvNYOBFlnlUvSt5zWNJdcUewR2OXC3UqqEf6GWj69p4Jz2yJOG2vKi5uhs9Pexcjm-3sWAM2mEQO5mNT8gWCSl7l2cA6lT07tb6fjW9202upkYmdC4wFRtzM"

    assert {:ok, _claims} =
             verify_cognito_jwt(jwt, keys_module: MockCognitoKeys, region: "", pool_id: "")
  end

  test "verify_elb_jwt/2 pulls key from key module and verifies the JWT" do
    jwt =
      "..KRXk2vfWreeDEb2xJJLkcQkbjb7YoEv9DMcmGZvkLDYrbtKaDNvwEjUdZ4HcV7bCYNXaFqQRzQ56sj8P0gDLHQ"

    assert {:ok, _claims} = verify_elb_jwt(jwt, keys_module: MockCognitoKeys, region: "")
  end
end
